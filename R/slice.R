...
slice_nearby <- function(data, ..., buffer = 1, n = 5){
  ...
  if (nrow(target) == 0){
    abort("No candidate fulfill the condition(s),
  ...
  cand <- data %>% filter(dplyr::between(.data$lat, lat_min, lat_max),
  ...
  out <- rbind(target %>% mutate(type = "selected"),
...
view_nearby <- function(data, origin, map = NULL){
  ...
  bbx <- attr(data, "bbox")
  all <- origin %>% filter(dplyr::between(.data$lat, bbx[3], bbx[4]),
                           dplyr::between(.data$long, bbx[[1]], bbx[[2]]))

  plot_map(map_data = map) +
    ggplot2::geom_point(data = all, ggplot2::aes(x = .data$long, y = .data$lat), color = "grey") +
    ggplot2::geom_point(data = data, ggplot2::aes(x = .data$long, y = .data$lat, color = .data$type)) +
    ggplot2::geom_rect(data = data, ggplot2::aes(xmin = bbx[1],
                                                 xmax = bbx[2],
                                                 ymin = bbx[3],
                                                 ymax = bbx[4]),
                       linetype = "dashed",
