...
The raw data (`oz_climate`) records daily precipitation and (maximum and minimum) temperature for  `r unique(oz_climate$station) %>% length()` stations in a tibble format but it is messy: 

```{r}
oz_climate
```

recordings for each station starts from different years, some even date back to `r range(oz_climate$date)[1]`. There are also missingness for some variables in some stations and sometimes a station could have a variable that is not recorded at all. To be able to produce the glyph map above, several cleaning tasks need to done: 
...
This set of operations gives us `r climate_clean %>% nrow()` stations to work with and below is a map of the location of these weather station.
