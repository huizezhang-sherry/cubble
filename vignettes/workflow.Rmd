...
Currently, verbs in the `dplyr` package supported by cubble includes `filter`, `mutate`, `summarise`. In the example below, we want to calculate the sum of precipitation and the average max/min temperature for each month after 2015. Since the calculation is time-specific, we start with the time-wise long form.
...
The summarised cubble can be switched back to the list-column format again to compute station-wise variables
...
With a tsibble structure, we can further make the summary above into monthly summar by using the `index_by` function in `tsibble`:
...
```{r}
oz_global2 %>% 
  add_missing_prct(vars = c("prcp", "tmin", "tmax")) %>% 
  add_missing_dscrb(cutoff = c(0, 0.95))
```
