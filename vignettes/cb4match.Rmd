...
```{r}
matched <- matched %>% filter(group <= 10)
```
...
river2 <- river %>% rename(id = station)
...
```{r}
res_sp <- match_spatial(river2, vic) %>% filter(group != 7)
sp1 <- res_sp %>% filter(type == "river")
sp2 <- res_sp %>% filter(type == "climate")

res <- match_sites(sp1, sp2,
                   temporal_by = c("Water_course_level" = "prcp"),
                   temporal_independent = "prcp",
                   temporal_n_highest = 50,
...
ggplot() + 
  geom_sf(data = vic_map) + 
  geom_point(data = res %>% as_tibble(), 
             aes(x = long, y = lat, color = type)) + 
  ggrepel::geom_label_repel(data = res %>%  filter(type == "river") %>% as_tibble(),
                            aes(x = long, y = lat, label = group)) + 
  scale_color_brewer(palette = "Dark2") + 
  ggplot2::theme_bw() +
