...
```{r}
res_sp <- match_spatial(vic, river, spatial_n_group = 10) 
res_sp2 <- res_sp %>% bind_rows() %>% filter(!group %in% c(5, 8))
res <- res_sp2 %>% group_split(group) %>% purrr::map(~.x %>% update_cubble() %>% match_temporal(
...
```{r}
res <- res %>% bind_rows() %>% filter(n_matches >= 21)
```
...
ggplot() + 
  geom_sf(data = vic_map) + 
  geom_point(data = res_sp %>% bind_rows() %>% as_tibble(), 
             aes(x = long, y = lat, color = type)) + 
  ggrepel::geom_label_repel(data = res_sp %>% bind_rows() %>% as_tibble(), min.segment.length = 0,
                            aes(x = long, y = lat, label = group)) + 
  scale_color_brewer(palette = "Dark2") + 
  ggplot2::theme_bw() +
