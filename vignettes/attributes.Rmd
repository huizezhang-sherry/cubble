...
* `leaves` contains the list of variables that are time-invariant to the "spot"; 
* `form` specifies whether the data object is in the nested or long form. 

## Nested cubble

A nested cubble is built on top of a `rowwise_df`, which comes with the `groups` attributes, as well as the usual `names`, `row.names`, and `class`. Note that the `class` attribute contains the history class that a `rowwise_df` is built upon: `tbl_df`, `tbl` and `data.frame`. 

This is an example of the attributes for a nested cubble: 
...
In this example, `station` is the variable that identifies the "spot" and two stations are recorded: `ASN00001019` and `ASN00002012`. 

The `rowwise_df` class uses a `group` attributes to ensure each row is in its own group and this structure makes it simpler to calculate on the list. For example calculating the total precipitation can be done by: 
...
A long form cubble is built from a `grouped_df`, instead of `rowwise_df`. `grouped_df` ensures each "spot" is in the same group when the 

<!-- `grouped_df` also has a `groups` attribute and there are 366 rows for each station since the data contains daily records in 2020 (there are 366 days in 2020!).  -->
