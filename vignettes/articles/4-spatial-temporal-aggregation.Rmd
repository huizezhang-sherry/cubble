---
title: "4.spatial-temporal-aggregation"
---
...
With package `s2`, we can compute the distance matrix between stations. We use k-means for a basic clustering to aggregate the stations in the spatial dimension based on the distance matrix. After several trials, 30 clusters work well for all the stations in Australia.
...
After this, there are several functionalities and plots that we hope to make: 
  ...
  - aggregate the series within the cluster (potentially further aggregation is needed on the original series itself)
  - look at how does the aggregated series look like on the map (with glyph map)
...
spatial_cluster <- station_clustered %>% 
  ...
  mutate(ll_cluster = s2_make_polygon(c(val$long[chull]), c(val$lat[chull]), oriented = FALSE),
...
ggsave(p1, filename = here::here("man/figures/spatial-temporal-agg-map.png"), 
       width = 5, height = 5)
```

```{r echo = FALSE}
knitr::include_graphics(here::here("man/figures/spatial-temporal-agg-map.png"))
```
...
```{r eval = FALSE, echo = FALSE}
ggsave(p2, filename = here::here("man/figures/spatial-temporal-agg-ind.png"), 
       width = 5, height = 5)
```

```{r echo = FALSE}
knitr::include_graphics(here::here("man/figures/spatial-temporal-agg-ind.png"))
```
...
```{r eval = FALSE, echo = FALSE}
ggsave(p3, filename = here::here("man/figures/spatial-temporal-diag.png"), 
       width =5, height = 5)
```

```{r echo = FALSE}
knitr::include_graphics(here::here("man/figures/spatial-temporal-diag.png"))
```
