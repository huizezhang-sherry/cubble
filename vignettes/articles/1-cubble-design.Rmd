...
A long form cubble is built from a `grouped_df`, instead of `rowwise_df`. `grouped_df` records  all the timesface_spatials from the same site in the same group. Below prints the attributes of a long cubble:
...
Apart from the usual `%@%` extraction of attributes, `index()`, `key_vars()`, `key_data()`, `coords()`, and `spatial()` can be used to extract the relevant component in both forms:
