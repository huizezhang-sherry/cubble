...
Global Historical Climatology Network (GHCN) provides daily climate measures from stations across the world. `weatherdata::climate_full` extracts daily precipitation and maximum/ minimum temperature from GHCN for 639 stations in Australia from 2016 to 2020. This is where these stations locate in an Australia map: 
...
```{r}
station_long <- weatherdata::climate_full %>% 
  stretch(ts) %>% 
...
You can also use `stretch()` to get the cluster level long cubble:

```{r}
cluster_long <- cluster_nested %>% stretch(ts)
cluster_long
```
